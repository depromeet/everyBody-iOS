...
extension PanoramaViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == topCollectionView {
            if editMode {
                if indexPath.row == 0 {
                let cell: CameraCollectionViewCell = collectionView.dequeueReusableCell(for: indexPath)
                return cell
                } else {
                    ...
                    cell.setPhotoCell(index: indexPath.row-1)
                    return cell
                }
            } else {
                let cell: TopCollectionViewCell = collectionView.dequeueReusableCell(for: indexPath)
                cell.selectedViewIsHidden(editMode: editMode)
                cell.setPhotoCell(index: indexPath.row)
                return cell
            }
